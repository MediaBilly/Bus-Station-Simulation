...
int main(int argc, char const *argv[])
{
  ...
  // Create logfile semaphore
  if (sem_init(&(sm->logfile),1,1) != 0) {
    perror("Could not initialize logfile semaphore:");
    exit(1);
  }

  // Create bay semaphores
  for (i = 0; i < BAYS; i++) {
    if (sem_init(&(sm->bay[i]),1,bayCap[i]) != 0) {
      perror("Could not initialize bay semaphore:");
      exit(1);
    }
  }
